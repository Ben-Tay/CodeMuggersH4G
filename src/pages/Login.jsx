...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
        e.preventDefault(); // get rid of button default refresh behaviour when submit

        try {
            // Fetch user data from open-source api
            ...
            if (user && userRole == "Resident") {
                // Navigate to the user profile page, passing the logged-in user info as state
                localStorage.setItem('user', JSON.stringify(user));

                navigate('/users/profile', { state: { user } });
            } else if (user && userRole == "Admin") {
                localStorage.setItem('user', JSON.stringify(user));
                // admin is found, then navigate to admin dashboard
                navigate('/admin');
            }
            else {
        ...
        <div className="container mx-auto px-4">
            <div className="flex items-start justify-between pt-16">
                ...
                <main className="flex-1 bg-white p-8 rounded-lg shadow-lg mr-8">
                    ...
                    <p className="text-center text-gray-600 mb-8">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Error, doloremque odio? Cupiditate eius dolores laborum libero aperiam enim dolore, recusandae similique tenetur aliquam dignissimos velit tempore voluptas. Adipisci, consectetur deleniti?</p>
                </main>
                <aside className="w-96 bg-indigo-50 p-8 rounded-lg shadow-lg">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700" htmlFor="userSelect">Choose a user role:</label>
                            <select 
